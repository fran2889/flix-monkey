...
For a surface with `showFadeToggle: true`, render the existing fade toggle in the rating overlay, but visually hide it by default. Reveal it when that same surface is hovered or contains keyboard focus (`:focus-within`). The hidden state uses opacity and disabled pointer events, preserving layout so the rating badge does not move when the toggle appears.
...
Hover is supplemented by `:focus-within` so a keyboard user can reveal and operate the control. The control keeps its tooltip and state labels. Pointer events are enabled only while the control is revealed.
