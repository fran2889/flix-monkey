---
title: Settings UI Design Specification
author: Mistral Vibe
date: 2026-08-23
---
...
Define the visual design and DOM structure for the Settings UI.

### 1.2 Goals

- Implement the defined DOM structure and CSS styling
...
settings-ui.js (unchanged)
    └── settings-view.js (refactored)
            ...
            └── styles.js (updated with new CSS)
```
...
3. `SettingsView` generates DOM matching the defined structure
...
| Current Class              | New Class                      | Action                                    |
| -------------------------- | ------------------------------ | ----------------------------------------- |
...
- **New Design:** Auto-save pattern (no Save button)
...
- **New Design:** Error messages displayed, invalid fields highlighted
- **Implementation:** Keep existing validation logic, update error class

### 4.3 Action Handlers

- **Current:** `clearCache`, `resetClients` callbacks via `SettingsUI`
- **New Design:** "Clear Cache" and "Reset Providers" buttons
...
- **New Design:** `status`, `status--error`, `status--success` classes
